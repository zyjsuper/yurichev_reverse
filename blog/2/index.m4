...
Information in this table is constructed using function ksmlrs() (at least for 10.2), which also calls ksmsplu(). The last function allocate memory chunks for the table, marking them as <pre>x$ksmsp lrstat</pre> and <pre>x$ksmsp lru</pre>, then copy the table, then calls memset() C function to zero it.
